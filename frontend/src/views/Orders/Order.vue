  ...
  <div class="container">
    <div class="row">
      <div class="col-12 text-center">
        <h4 class="pt-3">Purchase Property Details</h4>
      </div>
    </div>
    ...
    <div v-for="order in orderList" :key="order.pid" class="row mt-2 pt-3 justify-content-around">
      ...
      <div class="col-md-5 px-3">
        <div class="card-block px-3">
          <h6 class="card-title">
            <router-link v-bind:to="'/order/'+order.id">Purchase No : {{order.id}}</router-link>
          </h6>
          <p class="mb-0">{{order.totalItems}} item<span v-if="order.totalItems > 1">s</span></p>
          <p id="item-price" class="mb-0 font-weight-bold">My Purchase Price : $ {{order.totalCost}}</p>
          <p id="item-total-price">Purchased on : {{order.orderdate}}</p>
        </div>
      </div>
      <div class="col-2"></div>
